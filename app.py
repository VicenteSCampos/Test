...
class _Worker(QThread):
    ...
    def _call_claude(self, transcription: str, asignatura: str, fecha: str,
        ...
        prompt = (
            ...
            f"REGLA CRÍTICA: Usa ÚNICAMENTE la información presente en la transcripción y el PPT proporcionados. "
            f"No añadas datos, fechas, nombres, definiciones ni ejemplos que no aparezcan explícitamente en el material. "
            f"Si un dato es incierto o debes inferirlo, márcalo con ⚠️ al inicio de esa línea.\n\n"
            f"---\n"
            ...
            f"[Describe en orden cronológico cómo se desarrolló la clase. Si hay PPT, úsalo para complementar "
            f"y enriquecer la secuencia de temas de la transcripción. Usa una lista numerada donde cada punto "
            ...
            f"[Entre 5 y 8 preguntas sobre temas EXPLÍCITAMENTE cubiertos en la transcripción. "
            f"Usa EXACTAMENTE este formato para cada una. "
            f"La respuesta debe basarse solo en lo dicho en clase — si el tema no fue explicado con claridad, no lo incluyas como pregunta:]\n\n"
            f"> [!question]- [escribe aquí la pregunta]\n"
            f"> [escribe aquí la respuesta, citando solo lo explicado en la transcripción]\n\n"
            f"---\n"
            f"TRANSCRIPCIÓN:\n{transcription}"
            f"{ppt_block}"
        )
